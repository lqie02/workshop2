   ...
   <?php if (isset($_SESSION['items']) && !empty($_SESSION['items'])){?>
    <form action="pay.php" method="POST" class="pay">
     <thead>
      ...
      <?php $query = $db->query("SELECT * FROM item WHERE item_id='".$key."'");  
        if($query->num_rows > 0){
      ...
      ?>
         ...
         <td style="padding: 10px;"><img src="/fkfood<?php echo $query->row['image']; ?>" style="height: 150px;"></td>
         <td style="padding: 10px;"><h3><?php echo $query->row['itemName']; ?></h3></td>
         <td style="padding: 10px;"><h3>RM <?php echo $query->row['unitPrice']; ?></h3></td>
         <td><input type="text" name="qty[<?php echo $key;?>]" id="qty-<?php echo $key;?>" value="<?php echo $qty;?>" style="height: 32px;font-size: 16px;width: 100px;"><button type="button" data-toggle="tooltip" title="" class="btn btn-primary" onclick="update(<?php echo $key;?>);" data-original-title="Update" style="margin-top: 0px;"><i class="fa fa-refresh"></i></button></td>
         <td style="padding: 10px; font-weight: bold;"><h3>RM <?php echo $query->row['unitPrice']*$qty; ?></h3></td>
     ...
     <tfoot>
      ...
      <tr>
     	<td colspan="5" style="padding: 5px;text-align:right;"><h3 style="margin: 0px;">Subtotal Total : </h3></td>
